import {Component, Input, OnInit} from "@angular/core";
import {Card} from "../app.component";

@Component({
  selector: 'app-card',
  templateUrl: './card.component.html',
  styleUrls: ['./card.component.scss']
})
export class CardComponent implements OnInit{

  @Input() card: Card

  title = 'My Card Title'
  text = 'My sample text'

  cardDate = new Date()

  imgUrl = 'https://5.imimg.com/data5/SELLER/Default/2021/1/MB/EZ/LT/112040471/easywebes-png-500x500.png'
  newImgLogo = 'data:image/png;base64,'

  resultImg = ''
  disabled = false

  ngOnInit() {
    setInterval( () => {
    if(this.resultImg === this.imgUrl) {
      this.resultImg = this.newImgLogo
    } else {
      this.resultImg = this.imgUrl
    }

    this.disabled ? this.disabled = false : this.disabled = true
    }, 2000)
  }

  getInfo(): string {
    return 'This is info from function'
  }

  changeTitle() {
      this.card.title = 'Title has been changed!!!'
  }
}
